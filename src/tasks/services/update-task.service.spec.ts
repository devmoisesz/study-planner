...
describe("UpdateTaskService", () => {
    ...
    it("updates only the editable task fields", async () => {
        ...
        await expect(sut.execute("task-1", data)).resolves.toEqual({ id: "task-1", ...data });
        expect(tasksRepository.update).toHaveBeenCalledWith("task-1", data);
    });
    ...
    it("throws 404 when the task does not exist", async () => {
        ...
        await expect(sut.execute("missing-task", data)).rejects.toBeInstanceOf(NotFoundException);
    });
});
