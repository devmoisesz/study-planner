...
describe('Get Task Service', () => {
  it('returns a task with resources and productivity history', async () => {
    ...
    await expect(sut.execute('user-1', 'task-1')).resolves.toEqual(task);
    expect(tasksRepository.findDetailsById).toHaveBeenCalledWith('task-1', 'user-1');
  });
  ...
  it('throws 404 when the task does not exist', async () => {
    ...
    await expect(sut.execute('user-1', 'missing-task')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
});
