...
describe('Delete Task Service', () => {
  ...
  it('should be possible to delete an existing task', async () => {
    ...
    await sut.execute(userId, task.id);

    expect(tasksRepository.items).toHaveLength(0);
  });

  it('should not delete other tasks', async () => {
    const first = await createTaskService.execute(userId, {
      title: 'Estudar Trigonometria',
      importance: 9,
      domain: 3,
      urgency: 8,
    ...
    await createTaskService.execute(userId, {
    ...
    await sut.execute(userId, first.id);
  ...
  it('should throw when the task does not exist', async () => {
    await expect(
      sut.execute(userId, '11111111-1111-4111-8111-111111111111'),
