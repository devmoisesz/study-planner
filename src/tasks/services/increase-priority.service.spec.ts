...
describe("IncreasePriorityService", () => {
    it("increases the score by the supplied ratio", async () => {
        ...
        await expect(sut.execute("task-1", 0.5)).resolves.toEqual({ id: "task-1", score: 59 });
        expect(tasksRepository.changeScore).toHaveBeenCalledWith("task-1", 59);
    });
    ...
    it("caps the score at 100", async () => {
        ...
        await expect(sut.execute("task-1", 0.9)).resolves.toEqual({ id: "task-1", score: 100 });
    });
    ...
    it("throws 404 when the task does not exist", async () => {
        ...
        await expect(sut.execute("missing-task", 0.5)).rejects.toBeInstanceOf(NotFoundException);
    });
});
